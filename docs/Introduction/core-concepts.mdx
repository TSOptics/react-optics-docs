...
:::info typecast
Here we cast `onLowestDurability` to a total optic (`Optic<Weapon>` is the shorthand of `Optic<Weapon, total>`) because the `Weapon` component expects a total optic while `onLowestDurability` is a **partial** one.  
