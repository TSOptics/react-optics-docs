...
-   First thing is **immutability**: React relies on reference change to render so we cannot mutate our state but have to create a new version of it each time we make a change.  
    However these immutable updates are notoriously verbose and error-prone in Javascript. In non trivial applications where the state is several level deep updating it ends up in the **spread operator pyramid of doom**.  
...
While we can have relatively simple ones like just focusing on a specific field from a path, they are flexible enough to express complex data access with conditions or conversions, generalizing to multiple targets, filtering them and focusing back to a single value.  
